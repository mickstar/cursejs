...
var accountEndpoint = require('../endpoints/accounts-v1.js');
...
class Client extends EventEmitter {
    ...
    setVisibilityOffline(callback){
        //Define an empty void if no callback specified
        if(callback === undefined){
            callback = _ => {};
        }

        accountEndpoint.setStatus(this.token, "offline", this.machineKey, function(errors, answer){
            if(errors === null){
                winston.log('info', "client.setoffline", "set account visibility to offline");
                callback(null);
            } else {
                winston.log('info', "client.setoffline", "set offline failed", errors);
                callback(errors);
            }
        });
    }

    reloadContacts(callback){
        this._loadContacts(callback)
    }
